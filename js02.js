...
const RIGHTS_COST = 1250; //cost of reproduction rights
const TRAVEL_RATE = 2; //travel cost per mile
...
function getEstimate() {
      ...
      let disance = document.getElementById("photoDist").value;
      ...
      //Add the cost of distcance per photographer per mile
      totalCost += photographers * disance * TRAVEL_RATE;

      //Add the cost of the book if purchased
      totalCost += buyBook ? BOOK_COST : 0;

      //Add the cost of photo rights if purchased
      totalCost += buyRights ? RIGHTS_COST : 0;

      //Display the estmated total cost
